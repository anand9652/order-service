...
public class Main {
    public static void main(String[] args) {
        ...
        order1 = service.payOrder(order1.getId());
        System.out.println("✓ Order Paid: " + order1);
        printOrderStatus(order1);
        ...
        System.out.println("\n\n📦 SCENARIO 2: Order Cancellation (CREATED → CANCELLED)\n");
        System.out.println("─".repeat(60));
        ...
        System.out.println("\n⚠ Attempting transition from CANCELLED (terminal) → PAID");
        try {
            service.payOrder(order2.getId());
            System.out.println("❌ ERROR: Should have thrown InvalidTransitionException!");
        } catch (InvalidTransitionException e) {
            System.out.println("✓ Correctly rejected: " + e.getMessage());
        }

        // ========== Scenario 4: Payment then Cancel ==========
        System.out.println("\n\n📦 SCENARIO 3: Payment then Cancellation (CREATED → PAID → CANCELLED)\n");
        System.out.println("─".repeat(60));
        ...
        order3 = service.payOrder(order3.getId());
        System.out.println("✓ Order Paid: " + order3);
        printOrderStatus(order3);

        order3 = service.cancelOrder(order3.getId());
        System.out.println("✓ Order Cancelled from PAID: " + order3);
        printOrderStatus(order3);

        // ========== Scenario 5: Partial Lifecycle ==========
        System.out.println("\n\n📦 SCENARIO 4: Partial Lifecycle (CREATED → PAID → SHIPPED)\n");
        System.out.println("─".repeat(60));
        ...
        order4 = service.payOrder(order4.getId());
        System.out.println("✓ Order Paid: " + order4);
        printOrderStatus(order4);

        order4 = service.shipOrder(order4.getId());
        System.out.println("✓ Order Shipped: " + order4);
        printOrderStatus(order4);

        // ========== Scenario 6: Invalid State Transition ==========
        System.out.println("\n⚠ Attempting invalid transition: SHIPPED → PAID (reverse)");
        try {
            service.payOrder(order4.getId());
            System.out.println("❌ ERROR: Should have thrown InvalidTransitionException!");
        } catch (InvalidTransitionException e) {
            System.out.println("✓ Correctly rejected: " + e.getMessage());
        }
